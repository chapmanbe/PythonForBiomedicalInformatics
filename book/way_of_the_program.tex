...
an example of a \textbf{ high-level language}; other high-level languages
you might have heard of are C, C++, Perl, and Java.

There are
also \textbf{ low-level languages}, sometimes referred to as ``machine
...
are more likely to be correct.  Second, high-level languages are \textbf{
...
languages: \textbf{ interpreters} and \textbf{ compilers}.  An interpreter
...
called the \textbf{ source code}, and the translated program is called the
\textbf{ object code} or the \textbf{ executable}.  Once a program is
...
interpreter: \textbf{ interactive mode} and \textbf{ script mode}. In
interactive mode, you type Python programs and the interpreter displays
the result. To work in the interactive mode, you first start a Python \textbf{shell}. Python comes with a default shell, which can be started by typing \verb"python" in the system shell. There are also third-party shells such as IPython (which we will use extensively) or bpython. Once we've invoked a Python shell we can type Python statements and Python will provide the results:
...
\textbf{ prompt} the interpreter uses to indicate that it is ready.  If
you type \texttt{ 1 + 1}, the interpreter replies \texttt{ 2}. In the IPython shell, the \textbf{prompt} is \verb"In [1]:", \veb"In[2]:", \emph{etc.} where the number increases by one with each executed statement. Similarly, in IPython there is an output indicator \verb"Out[1]:"
\index{prompt}

Alternatively, you can store code in a file and use the interpreter to
execute the contents of the file, which is called a \textbf{ script}.  By
...
the file.  If you have a script named \texttt{ dinsdale.py} and you are
working in a UNIX command window, you type \texttt{ python
dinsdale.py}.  Similarly, youd could run the script using IPython by typing \texttt{ipython dinsdale.py}. In other development environments, the details of
...
when we talk about \textbf{ algorithms}.
...
structure.  \textbf{Finally, the details matter.  Small errors in
...
This program displays a value on the
...
obstinate people (Reeves and Nass, \emph{ The Media
...
For example, if you enter \texttt{ print} in the search window, the
first link that appears is the documentation of the \texttt{ print}
...
Start the Python interpreter and type \texttt{ help()} to start the online
help utility.  Or you can type \verb"help('print')" to get information
about the \texttt{ print} statement.
